...
print('\n\n @ @ @ @ @ @ START @ @ @ @ @ @ @ ');
print('file: ann_script_start.lua');
print('author Davide Chicco <davide.chicco@gmail.com>');
print(os.date("%c", os.time()));

require 'sys'
mkdirModelsCommand = 'mkdir -p models'
mkdirResultsCommand = 'mkdir -p ../results'
sys.execute(mkdirModelsCommand)
sys.execute(mkdirResultsCommand)
...
function executeTest(testPerceptron, dataset_patient_profile)
   ...
   require './metrics_ROC_PR_AUC_computer.lua'
   output_metrics_ROC_PR_AUC_computer = metrics_ROC_PR_AUC_computer(predictionTestVect, truthVect)
   thisROCAUC = output_metrics_ROC_PR_AUC_computer[1]
   thisPRAUC = output_metrics_ROC_PR_AUC_computer[2]

    local printValues = false
    local output_confusion_matrix = confusion_matrix(predictionTestVect, truthVect, THRESHOLD, printValues)
    -- {accuracy, arrayFPindices, arrayFPvalues, MatthewsCC, f1_score, tp_rate, tn_rate};
    
    thisAccuracy = output_confusion_matrix[1]
    thisMCC = output_confusion_matrix[4]
...
for exe=1,execution_number do
        ...
        for a=1,#hiddenUnitVect do
            ...
            for k=1,#train_patient_profile do
                ...
                io.write(" (epoch="..epoch..") loss = "..round(loss,3).." ")      
        ...
        -- thisMCC, thisAccuracy, thisF1score, thisTPrate, thisTNrate, thisROCAUC, thisPRAUC

        local lastMCC = executeTestOutput[1]
        local lastAccuracy = executeTestOutput[2]
        local lastF1score = executeTestOutput[3]
        local lastTPrate = executeTestOutput[4]
        ...
        global_lastF1score[exe] = lastF1score
        global_lastTPrate[exe] = lastTPrate
...
-- print("global_lastMCC")
-- print(global_lastMCC)
-- 
-- print("global_lastF1score")
-- print(global_lastF1score)
-- 
-- print("global_lastAccuracy")
-- print(global_lastAccuracy)
-- 
-- print("global_lastTPrate")
-- print(global_lastTPrate)
-- 
-- print("global_lastTNrate")
-- print(global_lastTNrate)
-- 
-- print("global_lastROCAUC")
-- print(global_lastROCAUC)
-- 
-- print("global_lastPRAUC")
-- print(global_lastPRAUC)

MCC_exe_mean = stats_mean(global_lastMCC)
accuracy_exe_mean = stats_mean(global_lastAccuracy)
F1score_exe_mean = stats_mean(global_lastF1score)
TPrate_exe_mean = stats_mean(global_lastTPrate)
